...
export default function AccountTypeSwitcher({
  ...
  const handleToggleAccountType = async () => {
    ...
    try {
      ...
      const endpoint = `${API_BASE_URL}/users/me/demo/${
        ...
        <div className="switch-real w-64 absolute left-0 top-15 bg-secondary rounded-md shadow-sm p-3 z-50">
          ...
          <div className="flex items-center justify-between py-2">
            ...
            <label className="relative inline-flex items-center cursor-pointer">
              ...
              <div
                className={`absolute ${
                  locale === "en" ? "right-0.5" : "left-[1.7rem]"
                } top-0.5 w-5 h-5 bg-primary-700 peer-checked:bg-gray-200 rounded-full transition duration-300`}
              ></div>
            </label>
          </div>
        </div>
