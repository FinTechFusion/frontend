...
function PlanContent({ selectedPlanType, excludedPlanId }: PlanCardProps) {
    ...
    if (accessToken) {
      ...
      try {
        ...
        if (result.success && result.data.client_secret) {
          router.push(
            `/site/payment?clientSecret=${encodeURIComponent(
  ...
  const filteredPlans = data
    ?.filter((plan: PlanType) => {
      return (
        plan.frequency === selectedPlanType ||
    ...
    <>
      ...
      <div className="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-6">
        {filteredPlans?.map((plan: PlanType, index: number) => (
          <div
            className="relative shadow-sm border border-gray-200 px-8 py-10 rounded-[14px] h-full"
            key={index}
          >
            <h2 className="text-2xl font-medium pb-4">
              {plan.id === "beginner_trial" ? t("freeTrial") : plan.name}
            </h2>
            {(plan?.id === "essential_monthly" ||
              plan?.id === "essential_yearly") && (
              <div
                className={`absolute top-0 ${
                  locale == "en" ? "right-0" : "left-0"
                } bg-primary-600 rounded`}
              >
                <p className="text-secondary text-lg p-2">{t("mostPopular")}</p>
              </div>
            )}
            <span className="text-gray-500 text-lg pt-5">
              <b className="text-gray-950 text-3xl">
                {plan.price}/
                {plan.frequency === "monthly" || plan.frequency === "trial"
                  ? t("mo")
                  : t("yearly")}
              </b>
            ...
            <div className="plan-include">
              <p className="text-gray-800 text-xl font-medium pt-4">
                {t("includes")}
              </p>
              <ul className="py-3">
                {plan?.features?.map((feature: string, index: number) => (
                  <li
                    key={index}
                    className="capitalize py-2 flex justify-start items-start"
                  >
                    <FaCheck className="pe-2 text-primary-600 text-2xl" />{" "}
                    {feature}
                  </li>
                ))}
              </ul>
