...
export default function BinancePopup() {
   ...
   const { user } = useAuth();
   const t = useTranslations("binanceBopup");
   useEffect(() => {
   ...
   return (
      <>
         {!(user?.is_binance) &&
            <div
               className={`fixed md:bottom-3 md:right-3 bottom-0 right-0 bg-secondary rounded-2xl border border-blue-100 bg-white p-4 shadow-lg sm:p-6 lg:p-8 md:w-1/3 z-50 transition-all duration-500 transform ${isVisible ? "translate-y-0 opacity-100" : "translate-y-10 hidden"
                  }`}
               role="alert"
            >
               <div className="flex items-center gap-4">
                  ...
                  <p className="font-medium sm:text-lg">{t("newMsg")}</p>
               </div>
               <p className="mt-2 text-lg text-gray-500 capitalize">{t("popMsg")}</p>
